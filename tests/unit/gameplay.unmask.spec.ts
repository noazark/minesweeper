import { unmask, pointToOffset } from '../../lib/gameplay'
...
describe('unmask', () => {
  ...
  describe('zero adjacent bombs', () => {
    ...
    it('result does not include bombs', () => {
      expect(unmasked).not.toContainEqual(pointToOffset(matrix, {r: 0, c: 2}))
    })
    ...
    it('result includes all adjacent tiles', () => {
      ...
      expect(unmasked).toContainEqual(pointToOffset(matrix, {r: 2, c: 0}))
    })
